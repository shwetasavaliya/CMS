...
export default class ProjectController {
  ...
  async getProject(@Req() request: any, @Res() response: any){
    try {
      ...
      const data:any = {};
      if(id){data._id = Mongoose.Types.ObjectId(id)}
      const frontEnd = [
        {
          '$match': data
        },
      ...
      return response.formatter.ok({ getProject }, true, "PROJECT_DISPLAY_SUCCESS");
      
    } catch (error) {
      console.log("ERR:: ", error);
      return response.formatter.error({}, false, "PROJECT_DISPLAY_FAILED", error);
    }
  }
  ...
  async updateProject(
    ...
    try {
      ...
      if(typeof currentStatus === 'boolean') update.currentStatus = currentStatus;
